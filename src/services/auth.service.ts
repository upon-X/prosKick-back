...
import { PlayerProfile, type IPlayerProfile } from '../models/player_profile.model';
...
export type UpdateProfileRequest = z.infer<typeof update_profile_request_schema>;
...
class AuthService {
   ...
   */
  async authenticate_with_google(id_token: string): Promise<AuthResult> {
    try {
      ...
      if (!user) {
        ...
        is_new_user = true;
        logger.info('Nuevo usuario creado', { user_id: user?._id, email: decoded_token.email });
      } else {
        ...
        if (decoded_token.name && (!user.name || user.name !== decoded_token.name)) {
        ...
        await user.save();
        logger.info('Usuario existente autenticado', { user_id: user._id, email: decoded_token.email });
      }
      ...
      let player_profile = await PlayerProfile.findOne({ user_id: user._id.toString() });
      
      if (!player_profile) {
        player_profile = await this.create_player_profile(user, decoded_token);
        logger.info('Perfil de jugador creado', { profile_id: player_profile?._id });
      }
   ...
   */
  private async create_player_profile(user: any, decoded_token: any): Promise<any> {
    const profile_data = {
   ...
   */
  async update_player_profile(user_id: string, update_data: UpdateProfileRequest): Promise<IPlayerProfile> {
    try {
      ...
      logger.info('Perfil actualizado', { user_id, profile_id: player_profile._id });
      return player_profile;
    } catch (error) {
   ...
   */
  async get_player_profile_by_user_id(user_id: string): Promise<IPlayerProfile | null> {
    try {
   ...
   */
  async is_handle_available(handle: string, exclude_user_id?: string): Promise<boolean> {
    try {
